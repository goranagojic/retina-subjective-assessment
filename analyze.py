...
@click.option("-o", "--output-dir", type=click.Path(exists=False), required=True,
              help="Where to save plots generated by the function. If directory does not exist, it will be "
                   "created.")
@click.option("-w", "--what", type=str, required=False, default='grading_score',
              help="What to plot (grading score distributions, diesease distribution...")
@click.option("-s", "--show", is_flag=True, help="Show plots.")
def plot(type, input_file, output_dir, show, what):
    ...
    if what == 'grading_score':
        ...
        if type == "boxplot":
            r = data.boxplot(by=['dataset', 'doctorID'], column=['grading_score'], grid=False, rot=45, figsize=(9.67, 8),
                             return_type='both')
            export_data(
                boxplot=r[0],
                out=output_dir / "doctor-dataset.dat",
                xtickmapping={
                    "(drive, 1)": "DRIVE, E1",
                    "(drive, 2)": "DRIVE, E2",
                    "(drive, 3)": "DRIVE, E3",
        ...
        if type == "boxplot":
            ...
            export_data(
                boxplot=r[0],
                out=output_dir / "doctor-network.dat"
            )
    ...
    elif what == 'disease_distribution':
        disease_tokens = list()
        for d in data['correctAnswers']:    # this is done to
            disease_tokens.extend(str(d).split(','))
        pd.Series(disease_tokens).value_counts().plot(kind='bar', rot=10)
        plt.savefig(str(output_dir / "disease_distribution.png"))
...
def calculate_stats(input_file):
    ...
    total = data["correctAnswers"].count()
    for answer in data["answer"].unique():
        s = data[data["correctAnswers"].str.contains(answer)]["answer"]
        c, t = s[s == answer].count(), s.count()
        if t == 0:  # happens when doctors select there is no disease present in the image
            print(f"Warning! Cannot calculate ACC for class {answer} (c = {c}, t = {t})")
        else:
            print(f"\tClass {answer}, ACC {(c/t) * 100} (c = {c}, t = {t}), total representation in dataset {(t/total)*100}")
